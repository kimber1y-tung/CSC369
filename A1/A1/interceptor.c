...
{
	//Delete the current pid from all sys call pid lists
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//if pid is being monitored, log system call information
	if((table[syscall].monitored == 2) || ((table[syscall].monitored == 1) && check_pid_monitored(syscall, current->pid))){
		log_message(current->pid, (long)syscall, (long)reg.bx, (long)reg.cx, (long)reg.dx, (long)reg.si, (long)reg.di, (long)reg.bp);
	}


	//Call original function, stored in table
	table[syscall].f(reg);
 ...
 *     a) Cannot de-intercept a system call that has not been intercepted yet. -> finished
 *     b) Cannot stop monitoring for a pid that is not being monitored, or if the 
 *        system call has not been intercepted yet. -> finished
 * ^ finished
 *
 * - Check for -EBUSY conditions:
 *     a) If intercepting a system call that is already intercepted. -> finished
 *     b) If monitoring a pid that is already being monitored. -> finished
 * ^ finished
 *
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    // check if the cmd arguments are valid
    ...
    if ((cmd == REQUEST_STOP_MONITORING) || (cmd == REQUEST_START_MONITORING)){
        if (pid != 0){// pid == 0 -> start/stop monitoring for all pids
            // pid cannot be negative and needs to belong to a valid process
            if ((pid < 0) || (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)){
                return -EINVAL;
            }
        }
        // if process not at root, check if the 'pid' requested is owned by the calling process
        // if pid == 0, calling process has to be at root
        if (((current_uid() != 0) && (check_pids_same_owner(pid, current->pid))) || ((pid == 0) && (current_uid() != 0))){
            return -EPERM;
        }
    ...
    // If the intercept command is given, replace the corresponding function to the given syscall in the syscall table
    // with our interceptor function
    if(cmd == REQUEST_SYSCALL_INTERCEPT){

        // we must be root, use current_uid()
        if (current_uid() != 0){
            return -EPERM;
        }
        // lock then unlock
        ...
        // store old sys call function in table struct

        table[syscall].f = sys_call_table[syscall];
        table[syscall].intercepted = 1;
        spin_unlock(&my_table_lock);

        //replace sys call function with interceptor function
        ...
        //restore old sys call function from table struct
        spin_lock(&sys_call_table_lock);
        set_addr_rw((unsigned long)sys_call_table);
        ...
        // check if monitoring a pid that is already monitored
        if (table[syscall].monitored == 2){
            spin_unlock(&my_table_lock);
            return -EBUSY;
		}

        // case 2: It is intercepted and our interceptor function gets called, in our interceptor check:
        // 1. Is this syscall being monitored for this pid OR is it being monitored for all pids?
        // monitoring all pids
        if (pid == 0){
            destroy_list(syscall);
            table[syscall].monitored = 2;
        }
        // 2. If either of these are true, then that means we want to intercept this syscall and do something.
		else{
            // check if a pid cannot be added to a monitored list
        ...
        // check if the system call has been intercepted
        if (table[syscall].intercepted == 0){
            spin_unlock(&my_table_lock);
            return -EINVAL;
        }

        // check if stop monitoring a pid thats not monitored
        if ((check_pid_monitored(syscall, pid) == 0) && (table[syscall].monitored != 2)){
            spin_unlock(&my_table_lock);
            return -EINVAL;
        }

        // check if a pid cannot be deleted
		if (del_pid_sysc(pid, syscall) != 0){
            return -EINVAL;
        }
		// If no longer monitoring any pids for syscall, update monitored flag
...
static int init_function(void) {
	...
	spin_lock(&my_table_lock);
	//Init all of the pid lists for each system call
	...
	//Store MY_CUSTOM_SYSCALL function
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	//Store __NR_exit_group function
	orig_exit_group = sys_call_table[__NR_exit_group];

	set_addr_rw((unsigned long)sys_call_table);
	//Hijack MY_CUSTOM_SYSCALL function
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	//Hijack __NR_exit_group function
...
{        
    ...
    // Make sure to deintercept all syscalls, and cleanup all pid lists. -> use destroy_list()
    spin_lock(&my_table_lock);
    for (i = 0; i < NR_syscalls; i++){
        // deintercept
